...
function set_env_vars() {
  ...
  if [[ -f "$FILE_ZSHRC" ]]; then
    ...
    if ! grep -qzo "$ENV_VARIABLES_TEXT" "$FILE_ZSHRC"; then
      pl "$env_vars_text" >>"$FILE_ZSHRC" || log_error_exit $ERROR_ENV_VARS_APPEND
    else
      log "environment variables already exist!"
    fi
  else
    # create file with text.
    pl "$env_vars_text" >"$FILE_ZSHRC" || log_error_exit $ERROR_ENV_VARS_WRITE
...
function set_config_dir() {
  ...
  if bool $is_fresh_install; then
    ...
    # local version=""
    #   pl "Version file exists"
    #   version=$(head -n 1 "$version_file")
    #   if [[ "$version" =~ $version_pattern ]]; then
    #     pl "Version file is valid"
    #     if [[ "$version" != "$INSTALLER_DWN_VERSION" ]]; then
    #       pl "Different version"
    #     fi
    #   else
    #     # do something if not valid version
    #   fi
    # else
    #   pl "Version file does not exist"
    # fi
...
pl "installed dwn"
pl "=> v$DWN_VERSION"
pl "to check if the installation was successful, use"
pl
pl "    dwn -v"
pl
